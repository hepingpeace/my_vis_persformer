...
class Laneline_loss_3D(nn.Module):
    ...
    计算锚点表示中预测线与地面真实线之间的损失。
    锚点表示是基于真实的三维X，Y，Z。

    loss = loss1 + loss2 + loss2
    loss1: cross entropy loss for lane type classification 车道类型分类交叉熵损失
    loss2: sum of geometric distance betwen 3D lane anchor points in X and Z offsets 三维巷道锚点间几何距离之和(X、Z偏移量)
    loss3: error in estimating pitch and camera heights  估计间距和相机高度误差
    """
...
class Laneline_loss_gflat(nn.Module):
    ...
    计算锚点表示中预测线与地面gt线之间的损失。
    锚点表示形式为平面地面空间X'、Y'和真实3D Z。 可视化v估计也包括在内。

    loss = loss0 + loss1 + loss2 + loss2
    loss0: cross entropy loss for lane point visibility 车道点能见度交叉熵损失
    loss1: cross entropy loss for lane type classification 车道类型分类交叉熵损失
    loss2: sum of geometric distance betwen 3D lane anchor points in X and Z offsets 三维巷道锚点间几何距离之和(X、Z偏移量)
    loss3: error in estimating pitch and camera heights 估计间距和相机高度误差
    """
...
class Laneline_loss_gflat_3D(nn.Module):
    ...
    计算锚点表示中预测线与地面gt线之间的损失。
    锚点表示形式为平面地面空间X'、Y'和real三维Z。 能见度估计也包括在内。
    将X'Y'和Z估计转化为实X，Y，以便与地面gt进行比较。
    将X'Y'和Z估计转化为实X，Y，以便与地面gt进行比较。 在X，Y空间中，额外的损耗将引导特征学习，以适应两个空间之间的几何约束
...
if __name__ == '__main__':
    ...
    print(loss) #  tensor(1908.6936, device='cuda:0')
    ...
    print(loss) # tensor(1866.6240, device='cuda:0')
    ...
    print(loss) # tensor(27022.1133, device='cuda:0')